...
Fallback Pipeline (3 Tiers):
  ...
  Tier 3 → Tavily + AWS Bedrock (web search + LLM extraction via Bedrock converse API)
...
# ---------------------------------------------------------------------------
# Environment / API Keys — AWS Bedrock
# ---------------------------------------------------------------------------
TAVILY_API_KEY: str     = os.environ.get("TAVILY_API_KEY", "")

# AWS credentials — loaded from environment (set in .env or shell)
AWS_ACCESS_KEY_ID:     str = os.environ.get("AWS_ACCESS_KEY_ID", "")
AWS_SECRET_ACCESS_KEY: str = os.environ.get("AWS_SECRET_ACCESS_KEY", "")
AWS_DEFAULT_REGION:    str = os.environ.get("AWS_DEFAULT_REGION", "us-east-1")

# Bedrock model ID — use a Foundation Model ID for on-demand access (no endpoint ARN needed)
# or set BEDROCK_ENDPOINT_ARN to your Marketplace / custom deployment ARN.
# Default: Amazon Nova Micro (cheapest, fastest for extraction tasks)
BEDROCK_MODEL_ID: str = os.environ.get(
    "BEDROCK_MODEL_ID",
    "amazon.nova-micro-v1:0",   # fast & cheap; swap to "amazon.nova-lite-v1:0" for better quality
)
BEDROCK_ENDPOINT_ARN: str = os.environ.get("BEDROCK_ENDPOINT_ARN", "")  # optional custom ARN


def _build_bedrock_client() -> Optional[object]:
    """
    Build and return a boto3 bedrock-runtime client using env credentials.
    Returns None (with a warning) if credentials are missing.
    """
    if not AWS_ACCESS_KEY_ID or not AWS_SECRET_ACCESS_KEY:
        logger.warning(
            "[Bedrock] AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY not set — "
            "Tier 3 LLM extraction will be skipped."
        )
        return None
    try:
        return boto3.client(
        ...
        logger.error("[Bedrock] Failed to create boto3 client: %s", exc)
        return None
...
def llm_extract_entity(snippets: list[str], entity_input: EntityInput) -> Optional[str]:
    """
    Pass Tavily snippets to AWS Bedrock via the converse API.
    Uses the model configured in BEDROCK_MODEL_ID (or BEDROCK_ENDPOINT_ARN for
    Marketplace / custom deployments).

    Extracts *only* the target entity name, or returns None if no equivalent exists.
    """
    if not snippets:
        return None
    ...
    # Resolve model identifier: prefer explicit endpoint ARN (Marketplace deployments)
    # over the default Foundation Model ID (on-demand access)
    model_id = BEDROCK_ENDPOINT_ARN if BEDROCK_ENDPOINT_ARN else BEDROCK_MODEL_ID
    ...
    user_prompt = (
        ...
        f"of '{entity_input.entity_name}' (a {entity_input.entity_type} from "
        f"{entity_input.source_region}) for the {entity_input.target_region} region?\n\n"
        f"{context_block}\n\n"
        f"Reply with ONLY the entity name."
    )

    # Build converse payload — matches the pattern in bedrock_model.py
    converse_kwargs = {
        "modelId"        : model_id,
    ...
    try:
        logger.info("[Bedrock] Invoking model '%s' for entity extraction...", model_id)
        response  = bedrock.converse(**converse_kwargs)
        extracted = response["output"]["message"]["content"][0]["text"].strip()
        usage     = response.get("usage", {})
        logger.info(
            "[Bedrock] Response received. Tokens — in: %s, out: %s",
            usage.get("inputTokens", "?"),
            usage.get("outputTokens", "?"),
        )
        logger.info("[Bedrock] Extracted entity name: '%s'", extracted)

        if not extracted or extracted.upper() == "NO_EQUIVALENT":
            return None
        return extracted
        ...
        logger.error("[Bedrock] No valid AWS credentials found.")
        return None
    except Exception as exc:
        logger.error("[Bedrock] Unexpected error during LLM extraction: %s", exc)
        return None
...
if __name__ == "__main__":
    ...
    print(f"  Bedrock Model  : {BEDROCK_ENDPOINT_ARN or BEDROCK_MODEL_ID}")
    print(f"  AWS Creds      : {'SET ✓' if AWS_ACCESS_KEY_ID else 'NOT SET ✗'}")
    ...
    # ---------------------------------------------------------------
    # Test 3: Kasavu Saree (Kerala) → Karnataka — cache miss → Bedrock
    # ---------------------------------------------------------------
